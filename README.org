...
#+DESCRIPTION: The Tyrany of the default configuration
...
* A rant about my Emacs config and why i became a tyrant
  ...
  manager, with its own internal scriping language (elisp in this case).

  Emacs lets you synthesise the output of many processes into a single
  text based visualisation, and define your own personal workflow, or
  workflow's pioneered in the emacs community. It allows you to work
  ...
  causes cognative dissonence. If it does not work in emacs, you want it
  to have emacs keybindings to keep the dissonence to a reasonable
  level.
  ...
  I've also realised that this Emacs is not an editor, as much as a lisp
  IDE that can be used to run lisp programs to help me edit anything. My
  configuration is in fact a lisp program running inside a relativly
  ...
  may seem counter-intuitive, but it seems to be true. Bouns feature:
  easier to replicate across machines.

  The biggest problem with the longevity of any of my zemacsian
  conifuration has been adaptability. Decisions made to patch up a
  feature or to make life a bit easier don't lend themselves to lengthy
  explination in elisp commentary. Also i find myself commenting out
  ...
  - Update 2:
  After blithley promising a release of the tyrant on reddit, i realise
...
*** Tyrant Framework Creation [f9] key
    A core part of the tyrant approach is to minimise yak shaving
    ...
    a process optimisation idea comes up i can sketch it in, or at
...
*** notes on org-mode tangle/untangle usage
    ...
    - use el-get bundle to initiailise
...
*** DONE Tryant Emacs version checking
...
*** DONE toc@ helper functions
  ...
  appropriatly and defined allongside the places that use them.
